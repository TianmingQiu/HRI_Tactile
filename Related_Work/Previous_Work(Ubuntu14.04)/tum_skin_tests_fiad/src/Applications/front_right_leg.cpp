...
int main( int argc, char** argv )
{
    ...
    ros::Publisher id_pub = nh.advertise<std_msgs::Int32MultiArray>("ids_front_right_leg",1000);
    ros::Publisher range_pub = nh.advertise<std_msgs::Float32MultiArray>("range_front_right_leg",1000);
    ros::Publisher force_pub = nh.advertise<std_msgs::Float32MultiArray>("force_front_right_leg",1000);
    ...
    while(ros::ok())
    {
        ...
        {
            if (range[i] >= 0.2)
                ROS_INFO_STREAM("Front and right leg: Cell " << i << " is being touched!");
        }
