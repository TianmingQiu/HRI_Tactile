...
i=0
hand=False
state=False #collecting data or not
begin_time=rospy.Time()
t=1
q=0
range_dim17=None
predict_on=False #predicting
fv_touch=None
...
def callback_range(data):
	...
	if q==0:#last predict is finished, need to reset the parameters
	...
	elif q==1:#collecting data or prediction , don't reset the parameters
		if state==False and hand==False:
			q=q-1
	# predict_on
	if state==False and hand==False and not accx_array==None:
		predict_on=True
	else: 
		predict_on=False
...
def reshape_for_fv(accx_array,accy_array,accz_array,force_1_array,force_2_array,force_3_array,range_array):
...
if __name__ == '__main__':
	...
	#rospy.spin()
	clf= training_data_prepare()#build training model
	tts = ALProxy("ALTextToSpeech",nao_ip, 9559)

	while not rospy.is_shutdown():
		if predict_on ==True:
			fv_touch=reshape_for_fv(accx_array,accy_array,accz_array,force_1_array,force_2_array,force_3_array,range_array)
			if (clf.predict(fv_touch)==1):
				rospy.loginfo("\n PUSH")
				tts.say("Pushing")
				#print '\n PUSH'
			elif (clf.predict(fv_touch)==2):
				rospy.loginfo("\n TAP")
				tts.say("Tapping")
				#print '\n TAP' 
			elif (clf.predict(fv_touch)==3):
				rospy.loginfo("\n POKE")
				tts.say("Poking")
				#print '\n TAP' 
			predict_on=False
