...
int main( int argc, char** argv )
{
    ...
    ros::Publisher id_pub = nh.advertise<std_msgs::Int32MultiArray>("ids_right_arm",1000);
    ros::Publisher range_pub = nh.advertise<std_msgs::Float32MultiArray>("range_right_arm",1000);
    ros::Publisher force_pub = nh.advertise<std_msgs::Float32MultiArray>("force_right_arm",1000);
    ...
    while(ros::ok())
    {
        ...
        {
            if (range[i] >= 0.2)
                ROS_INFO_STREAM("Right arm: Cell " << i << " is being touched!");
        }
