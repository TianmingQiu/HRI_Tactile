...
// Possible ways:
// 1. map
// 2. vector<tum_skin_msgs_fiad::SkinCell> cells(number_of_cells);  (#include<vector>)
...
{
	ros::init(argc,argv,"all_cells");
	ros::NodeHandle nh;

	ros::Publisher id_pub = nh.advertise<std_msgs::Int32MultiArray>("ids_all_cells",1000);
	ros::Publisher range_pub = nh.advertise<std_msgs::Float32MultiArray>("range_all_cells",1000);
	ros::Publisher force_pub = nh.advertise<std_msgs::Float32MultiArray>("force_all_cells",1000);
    ...
    int number_cells = 80;
	vector<tum_skin_msgs_fiad::SkinCell> cells(number_cells);
	...
	vector<int> ids(number_cells);
	vector<float> range(number_cells);
	vector<float> force(number_cells);
	...
	while(ros::ok())
	{
        ...
        {
            if (range[i] >= 0.2)
                ROS_INFO_STREAM("Cell " << i << " is being touched!");
        }
