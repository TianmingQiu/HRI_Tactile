...
int main( int argc, char** argv )
{
	...
	ros::init(argc,argv,"back_left_leg");
	ros::NodeHandle nh;

	ros::Publisher id_pub = nh.advertise<std_msgs::Int32MultiArray>("ids_back_left_leg",1000);
	ros::Publisher range_pub = nh.advertise<std_msgs::Float32MultiArray>("range_back_left_leg",1000);
	ros::Publisher force_pub = nh.advertise<std_msgs::Float32MultiArray>("force_back_left_leg",1000);
    ...
    while(ros::ok())
    {
        ...
        /*** Get data from each cell and transmit them into vector (with a for-loop): ***/
        ...
        {
            if (range[i] >= 0.2)
                ROS_INFO_STREAM("Back and left leg: Cell " << i << " is being touched!");
        }
