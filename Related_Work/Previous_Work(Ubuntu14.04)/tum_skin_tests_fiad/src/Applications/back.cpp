...
int main( int argc, char** argv )
{
	...
	ros::Publisher id_pub = nh.advertise<std_msgs::Int32MultiArray>("ids_back",1000);
	ros::Publisher range_pub = nh.advertise<std_msgs::Float32MultiArray>("range_back",1000);
	ros::Publisher force_pub = nh.advertise<std_msgs::Float32MultiArray>("force_back",1000);
    ...
    while(ros::ok())
    {
        ...
        {
            if (range[i] >= 0.2)
                ROS_INFO_STREAM("Back: Cell " << i << " is being touched!");
        }
