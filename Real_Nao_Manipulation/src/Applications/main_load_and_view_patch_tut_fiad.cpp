...
//    }
    ...
    msg.data = 1;
    double prox[14];
    for (int ledid = 0; ledid <= 13; ledid++) {
        patch.setLedColor(Skin::Cell::LedColor::Red, ledid+1);
    }
    while(ros::ok())
    {
        ...
        patch.publish(ros::Duration(0.1));
        ...
        for (int i = 0; i <= 11; i++) {
            prox[i] = patch.dataFromId(i+1).prox.at(0);
            // ROS_INFO_STREAM("Prox on Cell"<<i+1<<": "<<prox[i]);
            if (prox[i] > 0.2){
                ROS_INFO_STREAM("Touched");
                chatter_pub.publish(msg);
                for (int num = 0; num <= 10; num++){
                patch.setLedColor(Skin::Cell::LedColor::Green, i+1);
            }
                
                patch.setLedColor(Skin::Cell::LedColor::Red, i+1);
                break;
            }
        }
        ...
        // if (prox1 > 0.5){ROS_INFO_STREAM("Prox 1 on Cell ID 4: "<<prox1);}
        // ROS_INFO_STREAM("Prox 1 on Cell ID 4: "<<prox1);
