...
//    }
    ...
    while(ros::ok())
    {
        ...
        //     different sensor data can be read from: /usr/include/tumskin/SkinCore/Cell/Data.h
        //double force1 = patch.dataFromId(4).force.at(0);
        
        int cell_num = patch.numberOfCells();
        for(int cell_i = 0; cell_i <= cell_num; cell_i ++){
            double prox = patch.dataFromId(cell_i).prox.at(0);
            ROS_INFO_STREAM("Proxmity on Cell ID " << cell_i << ": " << prox);
        }
        ROS_INFO_STREAM("======================================================");
